...
namespace mtlpp
{
    ...
    public:
        ...
        Buffer NewBuffer(uint32_t length, ResourceOptionsVal options);
        Buffer NewBuffer(const void* pointer, uint32_t length, ResourceOptionsVal options);
        Buffer NewBuffer(void* pointer, uint32_t length, ResourceOptionsVal options, std::function<void (void* pointer, uint32_t length)> deallocator);
